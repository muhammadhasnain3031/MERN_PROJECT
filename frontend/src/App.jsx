...
// 🔥 PROFESSIONAL UPDATE: Live Backend URLs
const API_BASE_URL = "https://mern-project-92xy.vercel.app/api";
const UPLOADS_URL = "https://mern-project-92xy.vercel.app/uploads";
...
function App() {
  ...
  const handleEdit = (s) => {
    ...
    // Using live URL for edit preview
    setPreview(`${UPLOADS_URL}/${s.image}?v=${refreshKey}`)
    setImage(null)
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editId) {
        await axios.put(`${API_BASE_URL}/update/${editId}`, data)
      } else {
      ...
      alert("Action Failed: Check if backend is connected") 
    }
  ...
  // --- UI THEME & STYLES (NO CHANGES TO YOUR UI) ---
  const theme = {
  ...
  return (
    <div style={{ backgroundColor: theme.bg, minHeight: '100vh', color: theme.text, transition: '0.4s ease', fontFamily: "'Plus Jakarta Sans', sans-serif" }}>
      <style>{`
        ...
        @media (max-width: 992px) { .main-container { flex-direction: column; } .sidebar-glass { width: 100%; height: auto; position: relative; border-right: none; border-bottom: 1px solid ${theme.border}; padding: 30px 20px; } .directory-main { padding: 30px 20px !important; } }
      `}</style>
      ...
      <div className="main-container">
        <aside className="sidebar-glass">
          ...
          <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '22px' }}>
            <div onClick={() => fileInputRef.current.click()} style={{ width: '130px', height: '130px', borderRadius: '35px', border: `2px solid ${theme.accent}`, margin: '0 auto', cursor: 'pointer', overflow: 'hidden', display: 'flex', position: 'relative' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(420px, 1fr))', gap: '25px' }}>
            {students.map((s, idx) => (
              <div key={s._id} className="member-card" style={{ padding: '30px', borderRadius: '30px', display: 'flex', alignItems: 'center', animationDelay: `${idx * 0.05}s`, flexWrap: 'wrap', gap: '15px' }}>
                <img 
                  src={`${UPLOADS_URL}/${s.image}?v=${refreshKey}`} 
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '15px', paddingLeft: '20px', borderLeft: `1px solid ${theme.border}`, marginLeft: 'auto' }}>
                  ...
                  <button onClick={() => { if(window.confirm("Permanent Wipe?")) axios.delete(`${API_BASE_URL}/delete/${s._id}`).then(fetchStudents) }} style={{ border: 'none', background: 'none', color: '#ff4757', fontWeight: '900', cursor: 'pointer', fontSize: '11px' }}>WIPE</button>
                </div>
              </div>
            ))}
          </div>
